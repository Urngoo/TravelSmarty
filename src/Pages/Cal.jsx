...
const Cal = () => {
  ...
  const onDrop = (e, targetDate) => {
    ...
    setMeetingData((prev) => {
      const newData = { ...prev };
      const sourceMeetings = [...(prev[sourceDate] || [])];
      const draggedMeetingIndex = sourceMeetings.findIndex(
        (meeting) => meeting.id === meetingId
      );

      if (draggedMeetingIndex === -1) return prev;

      const [draggedMeeting] = sourceMeetings.splice(draggedMeetingIndex, 1);

      if (sourceMeetings.length === 0) {
        delete newData[sourceDate];
      } else {
        newData[sourceDate] = sourceMeetings;
      }

      newData[targetDate] = [...(prev[targetDate] || []), draggedMeeting];
      return newData;
    });
  };
  ...
  const getEventsList = () => {
    return Object.entries(meetingData)
      .filter(([date, meetings]) => meetings && meetings.length > 0)
      .flatMap(([date, meetings]) =>
        meetings.map((meeting) => ({
          key: meeting.id,
          date: date,
          text: meeting.text,
